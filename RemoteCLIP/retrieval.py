...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Path to original clip weight",
    )
...
class JsonDataset(Dataset):
    ...
    def read_json(self):
        ...
        for image in datasets['images']:
            if image['split'] == "test": # 寻找测试图像
                for text in image['sentences']:
                    self.images.append(image['filename'])  # 将图像名称添加到images
                    self.captions.append(text['raw'].capitalize())  # 将图像对应的5个句子添加到captions

    def duplicate(self):
        unique_images, indexs = np.unique(self.images, return_index=True) # 获取唯一的图像名称和下标
