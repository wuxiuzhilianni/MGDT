...
class RotatedARSLLoss(nn.Module):
    def __init__(self, cls_channels=15, loss_type='origin', bbox_loss_type='l1'):
    ...
    def hard_neg_mining(self,
        ...
        scales = torch.cat(scale_list, axis=0).to(pos_ind.device)
        ...
        # decode pos bbox
        # print(f'max_vals: {max_vals.device}\n')
        # print(f'scales device: {scales.device}\n')
        # print(f'pos_ind device: {pos_ind.device}\n')
    ...
    def forward(self, teacher_logits, student_logits, img_metas=None, **kwargs):
        ...
        # 同步loss，防止 AssertionError: loss log variables are different across GPUs!
        # for key in loss_all:
        #     all_reduce(loss_all[key], op=torch.distributed.ReduceOp.SUM)  # 使用分布式通信同步损失值
        #     loss_all[key] /= torch.distributed.get_world_size()  # 平均损失值
