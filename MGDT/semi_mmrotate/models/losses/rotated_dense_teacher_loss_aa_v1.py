...
class RotatedDTLossAssignerAssistentV1(nn.Module):
    def __init__(self, cls_channels=len(CLASSES), loss_type='origin', bbox_loss_type='l1'):
        super(RotatedDTLossAssignerAssistentV1, self).__init__()
        ...
        self.image_class_prompt_path = '/workspace/animax/MCL/tools/Assinger_Assistent/image_class_prompt_from_chat.pt'
    ...
    def forward(self, teacher_logits, student_logits, ratio=0.03, img_metas=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            # 按联合置信度找到最大值及其类别
            max_vals, max_inds = torch.max(teacher_probs, dim=1)  # (21824,), (21824,)


            # 优先选取 max_inds 属于 exist_classes 的 pixel
            valid_mask = torch.isin(max_inds, exist_classes_tensor)  # 属于存在类别的掩码
            valid_inds = torch.where(valid_mask)[0]  # 符合条件的索引

            if len(valid_inds) >= count_num:
                # 如果符合条件的候选框足够
                valid_vals = max_vals[valid_inds]
                sorted_vals, sorted_inds = torch.topk(valid_vals, count_num)
                selected_inds = valid_inds[sorted_inds]  # 选择前 count_num 个
            else:
                # 如果符合条件的候选框不足
                remaining_num = count_num - len(valid_inds)
                
                # 从不符合条件的候选框中选取
                non_valid_inds = torch.where(~valid_mask)[0]
                non_valid_vals = max_vals[non_valid_inds]
                non_sorted_vals, non_sorted_inds = torch.topk(non_valid_vals, remaining_num)
                
                # 合并两部分索引
                selected_inds = torch.cat([valid_inds, non_valid_inds[non_sorted_inds]])
            ...
            for cls_idx, cls_name in enumerate(CLASSES):
                # 仅统计被选中的点
                cls_mask = max_inds[selected_inds] == cls_idx  # 仅对 selected_inds 进行掩码
                cls_count = cls_mask.sum().item()  # 被选中的属于该类别的数量
                if cls_count > 0:
                    cls_vals_mean = max_vals[selected_inds][cls_mask].mean().item()
                else:
                    cls_vals_mean = 0.0
