...
class RotatedDTLossAssignerAssistentV2(nn.Module):
    def __init__(self, cls_channels=len(CLASSES), loss_type='origin', bbox_loss_type='l1'):
        super(RotatedDTLossAssignerAssistentV2, self).__init__()
    ...
    def forward(self, teacher_logits, student_logits, ratio=0.03, img_metas=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            # 优先选取 max_inds 属于 exist_classes 且 joint confidence 大于 0.02 的 pixel
            valid_mask = torch.isin(max_inds, exist_classes_tensor)  # 属于存在类别的掩码
            valid_confidence_mask = max_vals > 0.02  # 置信度大于0.02的掩码
            valid_mask = valid_mask & valid_confidence_mask  # 合并两个条件的掩码
