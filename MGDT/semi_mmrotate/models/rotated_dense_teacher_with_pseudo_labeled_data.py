...
class RotatedDenseTeacherWithPseudoLabeledData(RotatedSparseDetector):
    def __init__(self, model: dict, semi_loss_unsup, semi_loss_sup, train_cfg=None, test_cfg=None, symmetry_aware=False):
        super(RotatedDenseTeacherWithPseudoLabeledData, self).__init__(
        ...
        self.labeled_image_names = torch.load('/workspace/animax/MCL/tools/save_labeled_data_name/labeled_image_names_percent5.pt')

    def forward_train(self, imgs, img_metas, **kwargs):
        super(RotatedDenseTeacherWithPseudoLabeledData, self).forward_train(imgs, img_metas, **kwargs)
        ...
        if self.iter_count > self.burn_in_steps:
            ...
            with torch.no_grad():
                # get teacher data
                teacher_logits = self.teacher.forward_train(get_data=True, **format_data['unsup_weak'])
            # get student data
            student_logits = self.student.forward_train(get_data=True, **format_data['unsup_strong'])

            image_name = format_data['unsup_weak']['img_metas'][0]['ori_filename']
            if image_name not in self.labeled_image_names:
                unsup_losses = self.semi_loss_unsup(teacher_logits, student_logits, ratio=self.region_ratio, img_metas=format_data['unsup_weak'])
            else:
                unsup_losses = self.semi_loss_sup(teacher_logits, student_logits, ratio=self.region_ratio, img_metas=format_data['unsup_weak'])
