...
class MCLTeacher3Branch(RotatedSemiDetector):
    def __init__(self, model: dict, semi_loss, train_cfg=None, test_cfg=None):
        super(MCLTeacher3Branch, self).__init__(
        ...
        if train_cfg is not None:
            ...
            self.weakly_pesudo_weight = train_cfg.get("weakly_pesudo_weight", 0.5)
    ...
    def forward_train(self, imgs, img_metas, **kwargs):
        super(MCLTeacher3Branch, self).forward_train(imgs, img_metas, **kwargs)
        ...
        weakly_pesudo_losses = self.student.forward_train(**format_data['pesudo_weak'])

        for key, val in weakly_pesudo_losses.items():
            # if key[:4] == 'loss':
            if key == 'loss_cls':
                if isinstance(val, list):
                    losses[f"{key}_weakly_pesudo"] = [self.weakly_pesudo_weight * x for x in val]
                else:
                    losses[f"{key}_weakly_pesudo"] = self.weakly_pesudo_weight * val
            # else:
            #     losses[key] = val
