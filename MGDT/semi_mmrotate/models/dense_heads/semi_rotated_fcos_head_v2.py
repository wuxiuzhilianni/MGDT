...
class SemiRotatedFCOSHeadV2(RotatedFCOSHead):
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        #     # 计算每个采样点的采样面积（矩形面积 = stride_x * stride_y）
        #     sampling_areas = stride[..., 0] * stride[..., 1]  # shape: (num_points, num_gts)
        ...
        #     print("Number of points inside GT bbox (after center sampling):", num_inside) # 30 
        ...
        if self.center_sampling:
            ...
            n = 7 - 3 * (area / max_area).clamp(min=0, max=1)  # shape: (num_points, num_gts)

            # scale_factor范围[1.0, 1.8]，大目标缩放更大
            scale_factor = 2.2 - 1.0 * (area / max_area).clamp(min=0, max=1)
            # 保持宽高比的同时动态缩放
            ...
            area_compensation = 0.5 + 0.5 * area_ratio  # ∈[0.55,1.0]
