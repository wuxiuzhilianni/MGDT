...
class SemiRotatedFCOSHeadIOU(RotatedFCOSHead):
    ...
    def loss(self,
             ...
             gt_bboxes_ignore=None):
        """
        cls_scores, 类别的分类分数 D=15
        bbox_preds, 预测框 D=4
        angle_preds, 预测角度 D=1
        ious, centerness D=1
        上述 4 个值对于 5 张特征图中的所有像素都有对应的预测值
        特征图大小为 128,64,32,16,8
        """
        assert len(cls_scores) == len(bbox_preds) \
               == len(angle_preds) == len(ious)
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores] # 128,64,32,16,8
        ...
        pos_inds = ((flatten_labels >= 0)
                    & (flatten_labels < bg_class_ind)).nonzero().reshape(-1) # 获取正样本的下标
        num_pos = torch.tensor(
            len(pos_inds), dtype=torch.float, device=bbox_preds[0].device) # 正样本下标的数量
        ...
        if len(pos_inds) > 0:
            ...
            if self.separate_angle:
             ...
             # 如果没有正样本，初始化 pos_ious_targets
            pos_ious_targets = torch.zeros(len(pos_inds), device=bbox_preds[0].device)  # 或者根据需求初始化为其他值
