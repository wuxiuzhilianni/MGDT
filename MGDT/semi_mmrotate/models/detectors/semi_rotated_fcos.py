...
class SemiRotatedFCOS(RotatedFCOS):
    ...
    def forward_train(self,
        ...
        x = self.extract_feat(img) # FPN 的输出，[b, c, h, w] h, w 从128 到 8，c 固定为256
        if not get_pred:
            return self.bbox_head.forward_train(x, img_metas, gt_bboxes,
                                                gt_labels, gt_bboxes_ignore,
                                                get_data=get_data) # 返回的是一个字典，包含 loss 和 proposal
        with torch.no_grad():
            self.eval()
            bbox_results = self.simple_test(img, img_metas, rescale=True) # 返回的是一个字典，包含 bbox 和 label
            self.train()
        logits = self.bbox_head.forward_train(
            x, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore, get_data=get_data) # logits 包括每个 level 的 cls_score, bbox_pred, angle_pred, centerness  
